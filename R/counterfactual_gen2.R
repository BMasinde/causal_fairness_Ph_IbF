...
counterfactual_gen  <- function(df, tc){
  ...
  counterfactual_data  <- df %>%
    ...
    mutate(track_min_dist = min(track_min_dist, na.rm = TRUE),
           ...
           rain_blue_ls = rain_total * blue_ls_frac,
           rain_yellow_ls = rain_total * yellow_ls_frac,
           rain_orange_ls = rain_total * orange_ls_frac,
           rain_red_ls = rain_total * red_ls_frac
