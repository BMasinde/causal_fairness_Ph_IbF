---
title: 'Truncated model: Training and Testing Hazard Parent Nodes: Wind and Rain'
output:
...
```{r}
# Environment: Clear working environment
rm(list = ls())
...
trunc_wind_model_tuned <- train(
  wind_max ~ track_min_dist + island_groups, data = trunc_train,
  method = "rpart",
  trControl = control,
  tuneGrid = grid
)
...
```{r}
png(here("adjusted SCM/outputs", "dec_wind_trunc_model.png"), 
       width = 6, 
       units = "in",
       height = 4,
    res = 300)
...
#   rain_total ~ track_min_dist + island_groups, data = trunc_train,
#   method = "rpart",
#   trControl = control,
#   tuneGrid = grid
# )
